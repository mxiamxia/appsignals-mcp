...
async def test_list_application_signals_services_success():
    ...
    mock_response = {
        "Services": [
            {"ServiceType": {"Type": "web-service"}, "KeyAttributes": {"Environment": "prod", "Region": "us-east-1"}},
            {"ServiceType": {"Type": "api-gateway"}, "KeyAttributes": {"Stage": "v1"}},
        ]
    }
...
async def test_get_service_metrics_with_custom_metrics():
    ...
    with patch("boto3.client") as mock_boto:
        ...
        result = await call_tool(
            "get_service_metrics", {"service_name": "test-service", "metrics": ["Latency", "ErrorRate"]}
        )
